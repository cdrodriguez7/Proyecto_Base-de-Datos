...
DECLARE Cursorgenre CURSOR FOR
    SELECT DISTINCT CONVERT(REPLACE(REPLACE(genres, 'Science Fiction', 'Science-Fiction'),
        'TV Movie', 'TV-Movie') USING UTF8MB4) from movies.movie_dataset;
...
DECLARE CursorStatus CURSOR FOR
    SELECT DISTINCT CONVERT(status USING UTF8MB4) AS names from movies.movie_dataset;
...
DECLARE originalLanguage VARCHAR(2);
 -- Declarar el cursor
DECLARE CursorStatus CURSOR FOR
    SELECT DISTINCT CONVERT(original_language USING UTF8MB4) AS languages from movies.movie_dataset;
...
DECLARE CursorMovie CURSOR FOR
    SELECT id,`index`,budget,homepage, keywords, original_language, original_title, overview,
           ...
           vote_average, vote_count FROM movies.movie_dataset;
...
BEGIN
 ...
 DECLARE myCursor
  CURSOR FOR
   SELECT JSON_EXTRACT(CONVERT(production_companies USING UTF8MB4), '$[*]') FROM movies.movie_dataset ;
...
BEGIN
 ...
 DECLARE myCursor
  CURSOR FOR
   SELECT JSON_EXTRACT(CONVERT(production_countries USING UTF8MB4), '$[*]') FROM movies.movie_dataset ;
...
BEGIN
 ...
 DECLARE myCursor
  CURSOR FOR
   SELECT JSON_EXTRACT(CONVERT(spoken_languages USING UTF8), '$[*]') FROM movies.movie_dataset ;
...
BEGIN
 ...
 DECLARE myCursor
  CURSOR FOR
   SELECT id, production_companies FROM movies.movie_dataset;
...
BEGIN
 ...
 DECLARE myCursor
  CURSOR FOR
   SELECT id, production_countries FROM movies.movie_dataset;
...
BEGIN
 ...
 DECLARE myCursor
  CURSOR FOR
   SELECT id, spoken_languages FROM movies.movie_dataset;
